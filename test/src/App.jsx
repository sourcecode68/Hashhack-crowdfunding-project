...
function App() {
  ...
  const fetchContractDetailsFromBackend = async (address) => {
    try {
      const res = await fetch(
        `http://localhost:5000/contract-details?address=${address}`
      );
  ...
  const loadAllContracts = async () => {
    try {
      ...
      const detailsPromises = addresses.map((addr) =>
        fetchContractDetailsFromBackend(addr)
      );
        ...
        <div className="main-platform">
          ...
          <div className="body">
            ...
            <div className="fund-win hide-scrollbar">
              {contractDetails
                .filter(
                  (contract) => contract.title && contract.title !== "Untitled"
                )
                .map((contract, index) => (
